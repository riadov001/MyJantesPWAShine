...
import { Upload, X, Image, FileVideo, Camera, Send, CheckCircle, Car, ChevronDown, ChevronUp } from "lucide-react";
...
export default function Services() {
  ...
  const [vehicleFirstRegDate, setVehicleFirstRegDate] = useState("");
  ...
  const requestQuoteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setVehicleVin("");
      setVehicleFuelType("");
      setVehicleFiscalPower("");
      setVehicleFirstRegDate("");
      setVehicleColor("");
  ...
  const handleSubmit = () => {
    ...
    if (imageCount < 1) {
      toast({
        ...
        description: `Veuillez ajouter au moins 1 photo (${imageCount}/1)`,
        variant: "destructive",
      });
      return;
    }
    ...
    if (vehicleRegistration) vehicleData.vehicleRegistration = vehicleRegistration;
    if (vehicleMake) vehicleData.vehicleMake = vehicleMake;
    if (vehicleModel) vehicleData.vehicleModel = vehicleModel;
    if (vehicleVin) vehicleData.vehicleVin = vehicleVin;
    if (vehicleFuelType) vehicleData.vehicleFuelType = vehicleFuelType;
    if (vehicleFiscalPower) vehicleData.vehicleFiscalPower = vehicleFiscalPower;
    if (vehicleFirstRegDate) vehicleData.vehicleFirstRegDate = vehicleFirstRegDate;
    if (vehicleColor) vehicleData.vehicleColor = vehicleColor;
  ...
  const isFormValid = selectedServiceId && imageCount >= 1;
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-sm ${imageCount >= 1 ? 'text-green-600' : 'text-muted-foreground'}`}>
                {imageCount >= 1 ? (
          ...
          <Collapsible open={showVehicleInfo} onOpenChange={setShowVehicleInfo}>
            <CollapsibleTrigger asChild>
              <Button
                variant="outline"
                className="w-full justify-between"
                data-testid="button-toggle-vehicle-info"
              >
            ...
            <CollapsibleContent className="mt-3 space-y-3">
              <p className="text-xs text-muted-foreground">
                Ces informations peuvent être remplies automatiquement via le scan de votre carte grise depuis l'application mobile.
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label htmlFor="vehicle-reg" className="text-sm">Immatriculation</Label>
                  <Input
                    id="vehicle-reg"
                ...
                <div className="space-y-1">
                  <Label htmlFor="vehicle-make" className="text-sm">Marque</Label>
                  <Input
                    id="vehicle-make"
                    placeholder="Peugeot"
                    value={vehicleMake}
                ...
                <div className="space-y-1">
                  <Label htmlFor="vehicle-model" className="text-sm">Modèle</Label>
                  <Input
                    id="vehicle-model"
                    placeholder="308"
                    value={vehicleModel}
                ...
                <div className="space-y-1">
                  <Label htmlFor="vehicle-vin" className="text-sm">VIN</Label>
                  <Input
                    id="vehicle-vin"
                    placeholder="VF3XXXXX..."
                    value={vehicleVin}
                ...
                <div className="space-y-1">
                  <Label htmlFor="vehicle-fuel" className="text-sm">Carburant</Label>
                  <Input
                    id="vehicle-fuel"
                    placeholder="Diesel"
                ...
                <div className="space-y-1">
                  <Label htmlFor="vehicle-power" className="text-sm">Puissance fiscale</Label>
                  <Input
                    id="vehicle-power"
                    placeholder="7 CV"
                    value={vehicleFiscalPower}
                    onChange={(e) => setVehicleFiscalPower(e.target.value)}
                    data-testid="input-vehicle-fiscal-power"
                  />
                </div>
                <div className="space-y-1">
                  <Label htmlFor="vehicle-date" className="text-sm">1ère immatriculation</Label>
                  <Input
                    id="vehicle-date"
                    placeholder="01/01/2020"
                    value={vehicleFirstRegDate}
                    onChange={(e) => setVehicleFirstRegDate(e.target.value)}
                    data-testid="input-vehicle-first-reg-date"
                  />
                </div>
                <div className="space-y-1">
                  <Label htmlFor="vehicle-color" className="text-sm">Couleur</Label>
                  <Input
                    id="vehicle-color"
                    placeholder="Noir"
                    value={vehicleColor}
          ...
          {!isFormValid && (
            <p className="text-sm text-center text-muted-foreground">
              ...
              {selectedServiceId && imageCount < 1 && ` • Ajoutez ${1 - imageCount} photo(s) supplémentaire(s)`}
            </p>
          )}
        </CardContent>
      </Card>
