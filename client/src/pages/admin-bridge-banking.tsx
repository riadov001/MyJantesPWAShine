...
interface BunqStats {
  ...
  recent: { date: string; description: string; amount: string }[];
}
      ...
      return new Promise<BunqImportResult>((resolve, reject) => {
        ...
        reader.onload = async () => {
          try {
            ...
            const res = await apiRequest("POST", "/api/import-bunq", {
              base64,
              filename: file.name
            });
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => { navigator.clipboard.writeText(webhookUrl); toast({ title: "Copié !" }); }}
            >
              Copier
            </Button>
          </div>
...
export default function AdminBridgeBanking() {
  ...
  if (authLoading) return <div className="p-6"><Skeleton className="h-10 w-64" /></div>;
  if (!isAuthenticated || !isAdmin) return <div className="p-6 text-destructive">Accès non autorisé</div>;
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            OpenBanking & Import bancaire
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">Bridge API · Synchronisation comptable · Import bunq</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <CardContent className="pt-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-green-800 dark:text-green-300">
                {syncResult.imported} écriture(s) Bridge importée(s)
              </p>
        ...
        <TabsContent value="bridge" className="mt-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {/* Transactions du compte sélectionné */}
              <Card>
                <CardHeader className="pb-2">
