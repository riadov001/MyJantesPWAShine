...
export function CreateClientDialog({ open, onOpenChange, onClientCreated, initialData }: CreateClientDialogProps) {
  ...
  const [role, setRole] = useState<"client" | "client_professionnel">(initialData?.companyName ? "client_professionnel" : "client");
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.companyName) {
        ...
        setRole("client_professionnel");
      }
      if (initialData.siret) setSiret(initialData.siret);
      if (initialData.tvaNumber) setTvaNumber(initialData.tvaNumber);
      if (initialData.companyAddress) setCompanyAddress(initialData.companyAddress);
    }
  }, [initialData]);
