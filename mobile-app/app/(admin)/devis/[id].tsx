...
import type { QuoteDetail, QuoteStatus } from '@/lib/sdk';

const STATUS_OPTIONS: { key: QuoteStatus; label: string }[] = [
...
export default function AdminQuoteDetailScreen() {
  ...
  const statusMut = useMutation({
    mutationFn: (status: QuoteStatus) => client.adminUpdateQuoteStatus(id, status),
    onSuccess: (updated) => {
      qc.invalidateQueries({ queryKey: ['quote', id] });
      qc.invalidateQueries({ queryKey: ['admin-quotes'] });
      qc.invalidateQueries({ queryKey: ['admin-dashboard'] });
      setModalVisible(false);
      emitToast('success', `Statut mis à jour : ${quoteStatusLabel(updated.status)}`);
    },
    ...
    <SafeAreaView edges={['bottom']} style={{ flex: 1, backgroundColor: t.background }}>
      ...
      >
        <View style={styles.topRow}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.ref, { color: t.text }]}>
              {data.reference || `Devis #${data.id.slice(0, 8)}`}
            </Text>
            <Text style={[styles.date, { color: t.textMuted }]}>
              {formatDateFR(data.createdAt)}
            </Text>
          </View>
          <Badge label={quoteStatusLabel(data.status)} tone={statusTone(data.status)} />
        </View>
        ...
        <Card style={{ marginTop: spacing.md }}>
          ...
          <InfoRow label="Montant" value={data.quoteAmount ? formatEUR(data.quoteAmount) : null} />
        ...
        {data.items && data.items.length > 0 ? (
          <Card style={{ marginTop: spacing.md }}>
            <Text style={[styles.sectionTitle, { color: t.text }]}>Lignes</Text>
